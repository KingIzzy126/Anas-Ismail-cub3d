...
/*   Updated: 2025/07/14 14:39:44 by ialashqa         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	load_wall_textures(t_game *game)
{
	...
	while (i < 4)
	{
		...
		if (!game->textures[i].img)
			handle_texture_load_error(paths[i], game);
...
void	load_visor_textures(t_game *game)
{
	...
	while (i <= 5)
	{
		...
		if (!game->visors[i].img)
			handle_texture_load_error(v_path[i], game);
...
void	load_gun_textures(t_game *game)
{
	...
	if (!game->gun_normal.img)
		handle_texture_load_error("textures/weapons/rifle.xpm", game);
	...
	game->gun_shoot.img = mlx_xpm_file_to_image(game->mlx,
			"textures/weapons/rifle_shooting.xpm", &game->gun_shoot.width,
			&game->gun_shoot.height);
	if (!game->gun_shoot.img)
		handle_texture_load_error("textures/weapons/rifle_shooting.xpm", game);
